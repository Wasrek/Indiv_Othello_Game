...
public class GameState {
	...
	public List<Point> getCapturedPieces(Player player, Point move) {
		...
		for (int[] direction : directions) {
			List<Point> capturedInDirection = getCapturedInDirection(player, move, direction);
			capturedPieces.addAll(capturedInDirection);
		}

		return capturedPieces;
	}

	private List<Point> getCapturedInDirection(Player player, Point move, int[] direction) {
		List<Point> capturedInDirection = new ArrayList<>();
		int x = move.x + direction[0];
		int y = move.y + direction[1];

		while (isValidPosition(x, y) && getBoard()[x][y] == player.getOpponentCounter()) {
			capturedInDirection.add(new Point(x, y));
			x += direction[0];
			y += direction[1];
		}

		if (isValidPosition(x, y) && getBoard()[x][y] == player.getCounter()) {
			return capturedInDirection;
		}
	...
	private boolean isValidPosition(int x, int y) {
		return x >= 0 && y >= 0 && x < getBoardDims()[0] && y < getBoardDims()[1];
	}
}