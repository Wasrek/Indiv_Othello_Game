...
public class NewEvaluator extends Evaluator {
	...
	public float evaluate(GameState game, Player p) {
		...
		if (game.isOver()) {
			if (game.isWinning(p)) {
				// Adjust the score based on the difference in scores between the player and the opponent.
				score = (game.getBoardDims()[0] * game.getBoardDims()[1] * (float) EDGE_WEIGHT * (float) CORNER_WEIGHT) + (game.getScoreOfPlayer(p) - game.getScoreOfPlayer(game.getOpposingPlayer(p)));
			} else if (game.isWinning(game.getOpposingPlayer(p))) {
				score = ((-1 * game.getBoardDims()[0] * game.getBoardDims()[1]) * (float) EDGE_WEIGHT * (float) CORNER_WEIGHT) + (game.getScoreOfPlayer(p) - game.getScoreOfPlayer(game.getOpposingPlayer(p)));
			}
			return score;
		}
